...
async def test_waiter_observes_completed_result_without_local_event():
    ...
    try:
        result = await bus._await_result(correlation_id, asyncio.Event(), timeout_ms=200)
    finally:
        await task
...
async def test_timeout_keeps_pending_record_available_for_late_result():
    ...
    result = await bus._await_result(correlation_id, asyncio.Event(), timeout_ms=30)

    assert result.status == "timed_out"
    assert result.ok is False
    assert result.retryable is True
    assert result.detail == "Saga MQ response timeout"
    assert (await db.hgetall(pending_key))["status"] == "pending"
